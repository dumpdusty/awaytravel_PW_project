...
test('test_3.1 Check the store address', async function ({page}) {
    ...
    for (let i = 0; i < addressArray.length; i++) {
        await expect(page.locator('[class="content_tile_medium_diptych_dek__FvBYK"]', {hasText: `${addressArray[i]}`}))
            .toBeVisible()
    }
})
...
test('test_6.0 Given store`s title', async function ({page}) {
    await page.goto('https://www.awaytravel.com/')
    await page.locator('[class="nav_desktop_item_anchorContent__Nl3LP"]', {hasText: 'Stores'}).click()
...
test('test_6.1 store descriptions has the links', async function ({page}) {
    await page.goto('https://www.awaytravel.com/')
    await page.locator('[class="nav_desktop_item_anchorContent__Nl3LP"]', {hasText: 'Stores'}).click()
    await page.locator('[data-link="/stores/chicago"]').click()
...
test('test_7.0 Additional store information', async function ({page}) {
...
test('test_7.1', async function ({page}) {
    await page.goto('https://www.awaytravel.com/stores/austin')
...
test('test_8.0', async function ({page}) {
    ...
    await page.locator('div:nth-child(2) > .store_location_gallery_slideImage__hmQua > span > .image_component__IJv7W')
    await page.getByRole('button', {name: 'previous slide'}).click();
    await page.locator('.image_component__IJv7W').first().click();
    await page.getByRole('button', {name: 'previous slide'}).click();
    await page.locator('div:nth-child(7) > .store_location_gallery_slideImage__hmQua > span > .image_component__IJv7W')
})
